...
public class BijeliPijun : Figura
{
    public override void ProvjeriLegalnePoteze()
    {
        ...
        foreach (Polje item in Canvas.SvaPolja)
        {
            ...
            if (((this.odigraniPotezi == 0) && ((item.xRed == (this.xRed + 2)) && (item.yStupac == this.yStupac))) && (item.gameObject.transform.childCount == 0))
            {
                legalniPotezi.Add(new ObicanIliPosebanPotez(item));
            }
            else if ((item.xRed == 5) && ((Canvas.SvaPolja[item.xRed - 1, yStupac].gameObject.transform.childCount != 0) && (Canvas.SvaPolja[item.xRed - 1, item.yStupac].FiguraNaPolju is CrniPijun) && 
                (item.xRed == this.xRed + 1) && ((item.yStupac == this.yStupac- 1) || 
                (item.yStupac == this.yStupac + 1)) && (Canvas.PosljednjaPomaknutaFigura == Canvas.SvaPolja[item.xRed - 1, item.yStupac].FiguraNaPolju)))
            {
